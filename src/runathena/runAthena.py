...
# check source host    
from pandawnutil.checker import ParamChecker
if not ParamChecker.checkSourceHost(sourceURL):
    print 'ERROR: un-trusted host %s for input sandbox' % sourceURL
...
if directIn:
    if usePFCTurl:
        ...
        if lfcHost != '':
            ...
            lfcPyFile.write(tmpGetPfnLfcStr % (lfcHost,inputGUIDs+minbiasGUIDs+cavernGUIDs,oldPrefix,newPrefix))
            lfcPyFile.write("""
    ...
    if len(inputFiles)>0:
        ...
        if guidBoundary != [] and collRefName != '':
            ...
            for fileName in inputFiles:
                # skip DBR
                if fileName == dbrFile or fileName.startswith('DBRelease'):
                    continue
                # using EventLookup svc to get relation between TAG and parent
                if tagFileList != []:
                    # create symlink
                    if not directIn:
                        try:
                    ...
                    if not tmpLFNforPFC in tagFileList:
                        ...
                        continue
                # split
                com = 'CollSplitByGUID.exe -splitref %s -maxsplit 100000 -src PFN:%s RootCollection' % \
                      (collRefName,fileName)
                if tagFileList != []:
                    com += " -query '"
                    for tmpGUID in guidBoundary:
                        com += 'strstr(%s,"%s")>0 || ' % (collRefName,tmpGUID)
                    com = com[:-4]
                    com += "' " 
                print com
                ...
                for idxSubColl in subCollection:
                    ...
                    com = 'CollListFileGUID.exe -counts -queryopt %s -src PFN:%s RootCollection' % \
                          (collRefName,tmpName)
                    print com
                    status,out = commands.getstatusoutput(com)
                    print out
                    ...
                    if foundGUID != None:
                        # rename
			tmpBaseLFN = fileName.split('/')[-1]
                        tmpBaseLFN = re.sub('__DQ2-\d+$','',tmpBaseLFN)
                        newName = '%s.%s' % (tmpBaseLFN,tmpName)
                        os.rename(tmpName,newName)
                        # append
                        newInputFilesMap[foundGUID] = newName
                    else:
                        # remove
                        os.remove(tmpName)
            # get new list
            newInputFiles = []
            for tmpGUID in guidBoundary:
                if newInputFilesMap.has_key(tmpGUID):
                    newInputFiles.append(newInputFilesMap[tmpGUID])
            # use new list
            if newInputFiles == []:
                print "Empty input list after CollXYZFileGUID.exe"
                sys.exit(EC_Coll)
            inputFiles = newInputFiles
            # add references for old job which uses directIn for parent files
            if tagFileList == []:
...
outPickFile.close()
sys.exit(st)
""" % (lfcHost,poolRefs,oldPrefix,newPrefix,lfcOutPi)))
            lfcPyFile.close()
            # run LFC access in grid runtime
            lfcSh = '%s.sh' % commands.getoutput('uuidgen 2>/dev/null')
            if envvarFile != '':
...
# FileStager : disabled when tagFileList != [] since exacted TAGs are available locally
if useFileStager and directIn and tagFileList == []:
    try:
        ...
        for tmpLine in fsFH:
            ...
            if tmpMatch != None:
                ...
                if copyTool == 'lsm':
                    ...
                    fsStrs += '%sstagetool.CpCommand="./%s"\n' % (tmpMatch.group(1),pCopyToolName)
                elif newPrefix.startswith('/') or newPrefix.startswith('file:'):
                    # NAS
                    #copyToolCpCommand = "ln"
                    fsStrs += '%sstagetool.CpCommand="ln"\n' % tmpMatch.group(1)
                    fsStrs += '%sstagetool.CpArguments=["-s"]\n' % tmpMatch.group(1)
                else:
		    # keep original
                    fsStrs += tmpLine                
	...
	metaDataForFH = {}
	    ...
	    for tmpGUIDforMeta,tmpPFNforMeta in directPfnMap.iteritems():
            ...
            if tmpInputName.startswith('dcache:'):
                # remove dcache: prefix 
                tmpInputName = re.sub('dcache:','',tmpInputName)
            if tmpInputName.startswith('file://'):
                # remove file:// prefix 
                tmpInputName = re.sub('file://','',tmpInputName)
            # append checksum
            if tmpCheckSum != None:
...
if (not runTrf) and dbrFile == '':
    ...
    if not runTrf:
        ...
        tmpTrfFile.write("""if os.environ.has_key('DBRELEASE'):
